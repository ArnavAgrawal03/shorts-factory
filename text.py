...
starter_messages =[{ "role": "system", "content": "You are generating voiceover text for youtube shorts, the voiceover must be 20-35 seconds. It must also be sensational in nature"}]
...
def generate_quotes(client, people, save=True, filename="quotes.json", model="gpt-3.5-turbo", temperature=0.5):
    ...
    for person in people:
        ...
        new_messages.append({"role": "user", "content": prompt})
        chat_completion = client.chat.completions.create(model=model, messages=new_messages, temperature=temperature)
        response = f"Did you know {person} once said: " + chat_completion.choices[0].message.content + " subscribe for more quotes and facts!!"
